...
class _SolicitacoesState extends State<Solicitacoes> {
  ...
  @override
  void initState() {
    inicio = context.read<UserPontoManager>().usuario?.aponta?.datainicio ?? DateTime.now().subtract(const Duration(days: 15));
    fim = context.read<UserPontoManager>().usuario?.aponta?.datatermino ?? DateTime.now().add(const Duration(days: 15));
