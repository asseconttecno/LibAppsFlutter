                  ...
                  Container(height: 80, color: Colors.black,
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                      children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            File? img = await CameraService.getImage();
                            if(img != null){
                              carregar(context);
                              bool result = await context.read<CameraPontoManager>().setPhoto(
                                  context.read<UserPontoManager>().usuario!,
                                  img.readAsBytesSync(), null
                              );
                              if(result){
                                context.read<CameraPontoManager>().img = img.readAsBytesSync();
                                Navigator.pop(context);
                                Navigator.pop(context, context.read<CameraPontoManager>().img);
                              }else{
                                Navigator.pop(context);
                                CustomAlert.erro(
                                  context: context,
                                  mensage: 'Não foi possivel atualizar sua foto, tente novamente mais tarde!',
                                );
                              }
                            }
                          },
                        ...
                        GestureDetector(
                          onTap: () async {
                            File? img = await CameraService.getGallery();
                            if(img != null){
                              carregar(context);
                              bool result = await context.read<CameraPontoManager>().setPhoto(
                                  context.read<UserPontoManager>().usuario!,
