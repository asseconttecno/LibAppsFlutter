...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  Widget build(BuildContext context) {
    ...
    Form inputDatePicker() {
      //final _controller = TextEditingController(text: DateFormat('dd/MM/yyyy').format(_selectedDate.value));
      return Form(
        key: _formKey,
            ...
            child: Column(
              children: <Widget>[
                ...
                CustomTextFormField.custom(
                  ...
                  type: FormType.date, isBorder: true,
                  validator: (v){
                    if(v == null || v == '' || v.length != 10){
                      ...
                      final date = v.split('/');
                      final d = DateTime(int.parse(date.last),int.parse(date[1]),int.parse(date.first) );

                      if(!((d.isAfter(widget.firstDate) || d.isAtSameMomentAs(widget.firstDate)) &&
                        d.isBefore(widget.lastDate) || d.isAtSameMomentAs(widget.lastDate))){
                        return 'Digite uma data entre ${DateFormat('dd/MM/yyyy').format(widget.firstDate)} e ${DateFormat('dd/MM/yyyy').format(widget.lastDate)}';
                      }
                      return null;
                    }
                  }
    ...
    bool isDateGreaterOrEqual(DateTime date1, DateTime date2) {
      return date1.isAfter(date2) || date1.isAtSameMomentAs(date2);
    }

    bool isDateLessOrEqual(DateTime date1, DateTime date2) {
